<span class="tt_pg"><b>Saída Manual de Produtos</b></span><br><br>
<?php
/*------------------------ ALTERAÇÕES ------------------*\
#10/05/2016{
	-Desenvolvido;
...
if($salva == 1){
	...
	echo "<META http-equiv='refresh' content='0;URL=index.php?pg=s.produto'>";
	}
	else{
		$nqt = $pdt[qt]-$qt;
	$rs = mysql_query("update produtos set qt = $nqt where id = $id");
	mysql_query("insert into mvprodutos (data,us,pdt,qt,qtat,tp,acao) values (now(),'$cod_us','$id','$qt','$nqt','2','SAÍDA MANUAL')");
if($rs){
