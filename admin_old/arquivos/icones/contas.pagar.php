...
if ($salva == 1) {
    ...
    mysql_query("
        ...
        (now(),'2','$vlr','Contas a Pagar - $desc','$cod_us','1','$dt','$obs')
        ");
	echo "<script type='text/javascript'>alert('Agendamento Realizado com Sucesso.');</script>";
	echo "<META http-equiv='refresh' content='0;URL=index.php?pg=contas.pagar'>";
}
...
<img src="arquivos/icones/116.png" class="bt_p" style="position:absolute; top:5px; right:5px;" onclick="canPaga()">
<div id="pgdd"></div>
...
function paga(id){
	mostraMascara();
$.getJSON('retpaga.php?id='+id, function(pagaData){
	var vct = [];
	var motivo = [];
	var valor = [];
	var obs = [];
