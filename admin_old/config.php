...
## CONFIGURAÇÕES ##
//Desejamos um Feliz Aniversário repleto de bênçãos, que todos seus projetos realizem com sucesso conforme à vontade do Senhor. Felicidades!!! Família UMADTB.
...
## CONTAGEM DE ALERTAS ##
$mes = date('m');
//contar aniversariantes mês

$anMes = mysql_num_rows(mysql_query("select * from tb_inscritos where cidade = 'Telêmaco Borba' and date_format(nascimento,'%m') = '$mes'"));

$dia = date('d/m');
//contar aniversariantes dia
$anDia = mysql_num_rows(mysql_query("select * from tb_inscritos where cidade = 'Telêmaco Borba' and date_format(nascimento,'%d/%m') = '$dia'"));

## AUSENCIAS CONSECUTIVAS SEM TRATATIVA ##
$auCon = mysql_num_rows(mysql_query("select * from tb_consecutivo where ciente = 0"));
?>