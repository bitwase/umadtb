 ...
 * }
 ...
 #17/06/2016{
	 -Ajustar para informar valor corretamente no pagamento;
		-Verificar linha 471 - 480
	-Ajustado;
	}
 #21/06/2016{
	-Ajsutado informação onde não estava inserindo dados como realizado quando data da primeira percela era atual.
}
 #27/07/2016{
	 -Ajustar, quando valor é inteiro, está mostrando apenas XX, sem centavos;
	 -inserido loader enquanto carrega valor;
 }
 #29/07/2016{
	-colocar regra para que ao sair da página, verificar via ajax se foi selecionado um tipo de pagamento para o que estiver sendo feito, seja Agendamento, Venda ou Compra... caso não exista, automaticamente inserir como agendado apra a data atual... colocar opção nesta tela para agendar pagamento	
	
para garantir que será registrado este pagamento... colocar nova regra, quando abrir a página de pagamento, automaticamente inserir no financeiro como agendado para a data atual e pagamento à vista.

pegar o id desta inserção (o último), e salvar em variável... ao enviar os dados para o pagamento, pegar este id e remover, mantendo somente os valores inseridos pelo usuário... desta forma, caso o usuário esqueça de informar os dados, estará registado mesmo como em atraso.
 }
*/
...
$cm = $_REQUEST[cm];//adaptar métodos de pagamentos para informar pagamentos... nao esquecer de coocar como tipo de saída destes pagamentos.
$salva = $_POST[salva];

if($salva == 1){
	//verificar método de remover lançamento financeiro automático
//abaixo funcão que vai chamar a página para atualizar hist[orico]
if($cn != ""){
	mysql_query("delete from financeiro where tipo2 = 'A$cn'");
}
if($vn != ""){
	mysql_query("delete from financeiro where tipo2 = 'V$vn'");
}
if($cm != ""){
	mysql_query("delete from financeiro where tipo2 = 'C$cm'");
}
?>
<?php
...
if($tipo == 1){// se for pagamento a vista, dt1 recebe hoje
	...
	$vp = str_replace(',','',$vp);
		...
		(now(),'1','A$cn','$vp','Pagamento à vista do atendimento $cn.','$cod_us','1',now(),'')") or die(mysql_error());
	}//se consulta
	else if($vn!=""){//se venda
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vp','Pagamento à vista da Venda $vn.','$cod_us','1',now(),'')");
		mysql_query("update vendas set st = 3 where id = $vn");
	}//se venda
	else if($cm!=""){//se compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vp','Pagamento à vista da Compra $cm.','$cod_us','1',now(),'')");
		mysql_query("update compra set st = 3 where id = $cm");
	}//se compra
	}
	if($stp1 != $hoje){
		$vp = $_POST[vlReal];//valor original- valor que deverá ser pago
	$vp = str_replace(',','',$vp);
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vp','Agendamento de pagamento à vista do atendimento $cn.','$cod_us','1','$stp1','')") or die(mysql_error());
	}//se consulta
	else if($vn!=""){//se venda
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vp','Agendamento de pagamento à vista da Venda $vn.','$cod_us','1','$stp1','')");
		mysql_query("update vendas set st = 4 where id = $vn");
	}//se venda
	else if($cm!=""){//se compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vp','Agendamento de pagamento à vista da Compra $cm.','$cod_us','1','$stp1','')");
		mysql_query("update compra set st = 4 where id = $cm");
	}//se compra
	}
	}	
	if($tipo_pg == 2){
	...
	if($cn!=""){//se atendimento/consulta	
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vp','Pagamento em cheque da Compra $cm.','$cod_us','1','$dt_cheque','$obs')");
		mysql_query("update compra set st = 3 where id = $cm");
	}//se venda
	}	
	...
	if($tipo == 2){
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vlp','Pagamento 1/2 do atendimento $cn.','$cod_us','1','$stp1','')") or die("Erro.");
		}
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vlp','Pagamento 1/2 da Venda $vn.','$cod_us','1','$stp1','')") or die("Erro.");
		}
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 1/2 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}
		if($stp1 == $hoje){
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Pagamento 1/2 da Compra $cm.','$cod_us','2','$stp1','')") or die("Erro.");
		}
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 2/2 da Compra $cm.','$cod_us','1','$stp2','')");
		mysql_query("update compra set st = 4 where id = $cm");
		}//se compra
	}
	if($tipo == 3){
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 1/3 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}//se compra
		}
		if($stp1 == $hoje){
		if($cn!=""){//se atendimento/consulta
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vlp','Pagamento 1/3 do atendimento $cn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//se consulta
		if($vn!=""){//se venda
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vlp','Pagamento 1/3 da Venda $vn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//se venda
		if($cm!=""){//se compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Pagamento 1/3 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}//se venda
		}
		if($cn!=""){//se atendimento/consulta
		mysql_query("insert into financeiro 
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 2/3 da Compra $cm.','$cod_us','1','$stp2','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 3/3 da Compra $cm.','$cod_us','1','$stp3','')");
		mysql_query("update compra set st = 4 where id = $cm");
		}//se compra
	}
	if($tipo == 4){
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 1/4 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se compra
		}
		if($stp1 == $hoje){//colocar regra para validar se está sendo para um agendamento ou para uma compra
		if($cn != ""){//verifica se é consulta
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vlp','Pagamento 1/4 do atendimento $cn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se consulta
		if($vn != ""){//se venda
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vlp','Pagamento 1/4 da Venda $vn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se venda
		if($cm != ""){//se compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Pagamento 1/4 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se compra
		}
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 2/4 da Compra $cm.','$cod_us','1','$stp2','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 3/4 da Compra $cm.','$cod_us','1','$stp3','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 4/4 da Compra $cm.','$cod_us','1','$stp4','')");
		mysql_query("update compra set st = 4 where id = $cm");
		}//fim se compra
	}
	if($tipo == 5){
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vlp','Agendamento de pagamento 1/5 da Venda $vn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se for consulta/atendimento
		if($cm!=""){//para compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 1/5 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se for compra
		}
		if($stp1 == $hoje){
		if($cn!=""){//para atendimento
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vlp','Pagamento 1/5 do atendimento $cn.','$cod_us','1','$stp1','$obs')") or die("Erro.");
		}//fim se for consulta/atendimento
		if($vn!=""){//para venda
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','V$vn','$vlp','Pagamento 1/5 da Venda $vn.','$cod_us','1','$stp1','')") or die("Erro.");
		}//fim se for consulta/atendimento
		if($cm!=""){//para compra
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Pagamento 1/5 da Compra $cm.','$cod_us','1','$stp1','')") or die("Erro.");
		}		
	}
		if($cn!=""){//para consulta
		mysql_query("insert into financeiro 
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'1','A$cn','$vlp','Agendamento de pagamento 4/5 do atendimento $cn.','$cod_us','1','$stp4','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		...
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 2/5 da Compra $cm.','$cod_us','1','$stp2','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 3/5 da Compra $cm.','$cod_us','1','$stp3','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 4/5 da Compra $cm.','$cod_us','1','$stp4','')");
		mysql_query("insert into financeiro 
		(data,tipo,tipo2,valor,motivo,us,sit,dt_ag,obs) VALUES
		(now(),'2','C$cm','$vlp','Agendamento de pagamento 5/5 da Compra $cm.','$cod_us','1','$stp5','')");
		mysql_query("update compra set st = 4 where id = $cm");
		}//fim se compra
	}
}//se parcelado
...
if($cm != ""){
	echo "<script>
	alert('Pagamento informado com sucesso.');
	location.href = 'index.php?pg=cn.compra&vn=$cm';
	</script>";
	}
}//fim salva
...
join clientes cl on c.paciente = cl.id
...
<br> 
<span id="par1" style="display:none">
<input type="hidden" name="tipo_pg" value="1" id="pgDt1">
<?php
//datas 
$hoje=date("d/m/Y");
$hj_calc = date("Y-m-d");
...
?>
<b>1ª Parcela</b>:<input type="text" class="date" id="dtp1" onchange="data_pg('<?php echo $hoje;?>')" required name="dtp1" size="11" value="<?php echo $hoje?>"><br>
</span>
<span id="par2" style="display:none">
<b>2ª Parcela</b>:<input type="text" class="date" size="11" required name="dtp2" value="<?php echo $p2?>"><br>
</span>
<span id="par3" style="display:none">
<b>3ª Parcela</b>:<input type="text" class="date" size="11" required name="dtp3" value="<?php echo $p3?>"><br>
</span>
<span id="par4" style="display:none">
<b>4ª Parcela</b>:<input type="text" class="date" size="11" required name="dtp4" value="<?php echo $p4?>"><br>
</span>
<span id="par5" style="display:none">
<b>5ª Parcela</b>:<input type="text" class="date" size="11" required name="dtp5" value="<?php echo $p5?>"><br>
</span>
<input type="hidden" name="vlReal" id="pago" value="<?php echo $vo;?>">
<input type="submit" value="Finalizar" id="fin2" style="display:block">
</form>
<?php
...
document.getElementById('avs').onchange = function() {
	...
	var val1 = document.getElementById('vp').value;
...
var alt = 1;
	...
	if(hoje != dt_inf){
		document.getElementById("pgDt1").disabled = false;
	...
	if(hoje == dt_inf){
		document.getElementById("pgDt1").disabled = true;
...
document.av.vp.value = parseFloat(nvalor);
document.av.pago.value = nvalor.toFixed(2);
...
function calc_troco()
{
	...
	var nvalor2 = parseFloat(vl_troco);
	nvalor2 = nvalor2.toFixed(2);
...
function reseta_valores(){
	<?php
if($cm != ""){ //função para compra... valor recebido recebe valor total... troco recebe zero... se alterar, chamar novamente a função ?>
	document.getElementById("total_rec").value = document.getElementById("total_pagar").value;
	document.getElementById("secompra").style.display = 'none';
<?php }?>
...
function garantia(num,tipo){
	//receber tipo
	//pegar valor
	//pegar data atual
	var valor = document.getElementById("pago").value;
	//chamar a tela para pagamento 
	$.ajax({
      url:'agendapg.php?tp='+tipo+'&n='+num+'&valor='+valor,
       complete: function (response) {
//alert(response.responseText);
      },
      error: function () {
...
<?php
if($cn != "" && $salva == ""){
	echo "<script>garantia($cn,1);</script>";
}
else if($vn != "" && $salva == ""){
	echo "<script>garantia($vn,2);</script>";
}
if($cm != "" && $salva == ""){
	echo "<script>garantia($cm,3);</script>";
}
?>